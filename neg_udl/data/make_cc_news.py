...
def process_cc_news(args: tuple, save_step: int = 2000) -> None:
    """Pre-Process CC-News Dataset.

    Function splits data from cc_news dataset into sentences and
    ...
    for i in range(*ind_range):
        obj = nlp(dataset_cc_news['train'][i]['text'])
    ...
    pd.DataFrame(counter_dict).to_csv(os.path.join(target_path, f"cc_news-{p_no}.csv"))
...
def main(cfg: DictConfig) -> None:
    ...
    pathlib.Path(cfg.preprocessing.cc_news.target).mkdir(parents=True, exist_ok=True)
    with Pool(p) as pool:
        args: list = list(zip(
            range(1, n + 1),
            [cfg.preprocessing.cc_news.target] * p,
            get_ranges(n, p)))
        pool.map(process_cc_news, args)
